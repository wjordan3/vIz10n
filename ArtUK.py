...
for i in range(10,11):
    url = 'https://artuk.org/discover/artworks/view_as/grid/search/-54216/page/' + str(i)
    ...
    ptgs = open('paintings_UK.csv', 'a', encoding='utf-8')
    cnt = 0
    print(len(linkList))
    for lnk in linkList:
        print(cnt)
        try:
            ...
            #print(artist)
            name = title[0:15].replace(' ', '-') + '_' + artist[0:10].replace(' ', '-') +'_' + str(cnt) + '_UK'
            #print(name)
...
#urlretrieve(imgAdr, os.path.basename("test.jpg"))
#print(imgAdr)